...
version string (previously the hardcoded `APP_VERSION` literal in main.py)
and the "Novedades" modal's content - and renders its most recent entry to
sanitized HTML.

CHANGELOG.md lives at the repo root (GitHub convention). The Dockerfile does
`COPY app /app/app` with no extra nesting (unlike GamePanel/WebPanel's
`dashboard/` prefix), so the path from this file up to the repo root is the
same number of levels in both the local/dev checkout and the built image -
`_find_changelog()` only needs one candidate.
"""
...
def _find_changelog() -> Path:
    ...
    candidate = here.parents[2] / "CHANGELOG.md"  # app/services -> app -> repo root (or /app in the image)
    if not candidate.exists():
        raise FileNotFoundError(f"CHANGELOG.md no encontrado en {candidate}")
    return candidate
